...
groups = as.factor(rep(1:5, each = 5))
```
...
groups = as.factor(rep(1:2, each = 4))
```
...
```{r}
fplim = 0 # Limit threshold for false positive rate
...
g <- g + ggtitle(title_roc) +
  ...
  theme(panel.background = element_blank(),
        ...
        aspect.ratio = 1) + 
  xlim(c(1, fplim)) 
ggsave(file.path(path2res, paste0("roc_curve_", data.name, ".pdf")),
  width = 5, height = 5,
  device = "pdf") 
```
...
df.aucprs = data.frame("methods"=names(aucprs.c), "AUCPR" = round(aucprs.c*10000)/100)
df.aucprs <- df.aucprs[order(df.aucprs$methods),]
write.table(df.aucprs, 
            file = file.path(path2res, paste0("aucprs_", data.name, ".csv")),
            row.names = F, 
            sep = ";")
aucprs.c
```